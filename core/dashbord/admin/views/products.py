...
from dashbord.admin.forms import *
...
class AdminProductListView(LoginRequiredMixin, HasAdminAccessPermission, ListView):
    template_name = "dashbord/admin/products/product-list.html"
    paginate_by = 10
...
class AdminProductCreateView(LoginRequiredMixin, HasAdminAccessPermission, SuccessMessageMixin, CreateView):
    template_name = "dashbord/admin/products/product-create.html"
    queryset = ProductModel.objects.all()
    ...
    def form_valid(self, form):
        ...
        return redirect(reverse_lazy("dashbord:admin:product-edit", kwargs={"pk": form.instance.pk}))

    def get_success_url(self):
        return reverse_lazy("dashbord:admin:product-list")


class AdminProductEditView(LoginRequiredMixin, HasAdminAccessPermission, SuccessMessageMixin, UpdateView):
    template_name = "dashbord/admin/products/product-edit.html"
    queryset = ProductModel.objects.all()
    form_class = ProductForm
    success_message = "ویرایش محصول با موفقیت انجام شد"

    def get_success_url(self):
        return reverse_lazy("dashbord:admin:product-edit", kwargs={"pk": self.get_object().pk})
...
class AdminProductDeleteView(LoginRequiredMixin, HasAdminAccessPermission, SuccessMessageMixin, DeleteView):
    template_name = "dashbord/admin/products/product-delete.html"
    queryset = ProductModel.objects.all()
    success_url = reverse_lazy("dashbord:admin:product-list")
...
class AdminProductAddImageView(LoginRequiredMixin, HasAdminAccessPermission, CreateView):
    ...
    def get_success_url(self):
        return reverse_lazy('dashbord:admin:product-edit', kwargs={'pk': self.kwargs.get('pk')})
    ...
    def form_invalid(self, form):
        ...
        return redirect(reverse_lazy('dashbord:admin:product-edit', kwargs={'pk': self.kwargs.get('pk')}))
